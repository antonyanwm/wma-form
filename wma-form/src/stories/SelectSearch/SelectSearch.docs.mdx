import { Meta, Canvas, Story, ArgsTable } from '@storybook/blocks';
import * as SelectSearchStories from './SelectSearch.stories';

<Meta of={SelectSearchStories} />

# SelectSearch - Универсальный компонент селекта с поиском

## 🎯 Обзор

`SelectSearch` - это мощный и гибкий компонент селекта с поиском, который предоставляет полный набор возможностей для работы с выпадающими списками в современных веб-приложениях.

### ✨ Основные возможности

-   🔍 **Интеллектуальный поиск** - фильтрация опций по введенному тексту с поддержкой debounce
-   🎯 **Два режима выбора** - одиночный и множественный выбор с настраиваемым поведением
-   ⚡ **Асинхронная загрузка** - динамическая подгрузка данных с кэшированием и отменой запросов
-   🎛️ **Внешнее управление** - полный программный контроль через API хуки
-   📱 **Адаптивность** - корректная работа на всех устройствах и размерах экрана
-   🎨 **Кастомизация** - настраиваемые стили через CSS переменные
-   ⌨️ **Клавиатурная навигация** - полная поддержка доступности (a11y)
-   🏷️ **Умные теги** - отображение выбранных элементов с overflow и анимациями

## 📦 Установка и импорт

```tsx
import { SelectSearch, Form } from 'wma-form';
import 'wma-form/dist/index.css'; // Не забудьте импортировать стили
```

## 🚀 Быстрый старт

### Базовое использование

```tsx
import { SelectSearch, Form } from 'wma-form';

const options = [
	{ label: 'JavaScript', value: 'js' },
	{ label: 'TypeScript', value: 'ts' },
	{ label: 'Python', value: 'py' },
];

function App() {
	return (
		<Form
			formId='my-form'
			onValidSubmit={(data) => console.log(data)}>
			<SelectSearch
				formId='my-form'
				name='language'
				label='Выберите язык программирования'
				options={options}
				required={true}
			/>
		</Form>
	);
}
```

## 📚 API Reference

### Основные пропсы

<ArgsTable of={SelectSearchStories.Basic} />

### Типы данных

```tsx
interface Option {
	label: string; // Отображаемый текст
	value: string | number; // Значение опции
	[key: string]: any; // Дополнительные поля
}

interface SelectSearchProps {
	// Обязательные пропсы
	formId: string; // ID формы (должен совпадать с Form)
	name: string; // Имя поля
	label: string; // Подпись поля

	// Данные
	options?: Option[]; // Статические опции
	loadOptions?: (input: string) => Promise<Option[]>; // Асинхронная загрузка

	// Поведение
	multiple?: boolean; // Режим множественного выбора
	placeholder?: boolean | string; // Плейсхолдер
	required?: boolean; // Обязательное поле
	disabled?: boolean; // Отключенное состояние

	// Иконки
	prefix?: React.ReactNode; // Компонент перед полем ввода
	suffix?: React.ReactNode; // Компонент после поля ввода
	showDefaultSuffix?: boolean; // Показывать дефолтную стрелку

	// Валидация
	validator?: ({ value, name, opts }: { value: string; name?: string; opts?: any }) => string;

	// Начальные значения
	initialValue?: string | string[];

	// UI настройки
	className?: string; // Дополнительные CSS классы
	icon?: string; // Иконка для поля
	visibleCount?: number; // Количество видимых опций
	hideInput?: boolean; // Скрыть поле ввода
	debounceMs?: number; // Задержка поиска (мс)
	hideErrorOnFocus?: boolean; // Скрывать ошибку при фокусе

	// Множественный выбор
	maxVisibleItems?: number; // Макс. видимых тегов
	allowBackspaceDelete?: boolean; // Удаление через Backspace
	hideSelectedFromDropdown?: boolean; // Скрыть выбранные из дропдауна
	hideClearButton?: boolean; // Скрыть кнопку очистки
}
```

## 💡 Примеры использования

### 1. Простой селект

<Canvas of={SelectSearchStories.Basic} />

### 2. С prefix и suffix иконками

```tsx
// С React компонентами
<SelectSearch
	formId='form'
	name='search'
	label='Поиск пользователей'
	options={users}
	prefix={<Icon name="search" />}
	suffix={<Icon name="arrow-down" />}
/>

// С эмодзи
<SelectSearch
	formId='form'
	name='category'
	label='Категория'
	options={categories}
	prefix="🔍"
	suffix="🚀"
	showDefaultSuffix={true} // Дефолтная стрелка с ротацией
/>

// Без иконок
<SelectSearch
	formId='form'
	name='simple'
	label='Простой селект'
	options={options}
	showDefaultSuffix={false} // Никаких иконок справа
/>
```

### 3. Внешнее управление

<Canvas of={SelectSearchStories.ExternalControl} />

## 🎣 Хуки для внешнего управления

### useSelectExternalControl

Основной хук для программного управления компонентом:

```tsx
import { useSelectExternalControl } from 'wma-form';

function MyComponent() {
	const {
		// Основные методы управления
		selectByValue, // Выбрать опцию по значению
		clearAll, // Очистить все выбранные значения
		openDropdown, // Открыть дропдаун
		closeDropdown, // Закрыть дропдаун
		setSearchQuery, // Установить поисковый запрос

		// Получение данных
		getSelectedValues, // Получить выбранные значения
		getSelectedCount, // Получить количество выбранных
		hasSelectedValues, // Проверить наличие выбранных значений
		getFieldState, // Получить состояние поля
		isOptionSelected, // Проверить, выбрана ли опция

		// Управление фокусом
		focusField, // Установить фокус на поле
		blurField, // Убрать фокус с поля

		// Валидация
		validateField, // Валидировать поле
		markAsTouched, // Отметить поле как touched

		// Дополнительные методы
		selectMultiple, // Выбрать несколько опций
		removeOption, // Удалить конкретную опцию
		getCurrentQuery, // Получить текущий поисковый запрос
		isDropdownOpen, // Проверить, открыт ли дропдаун
	} = useSelectExternalControl({
		formId: 'my-form',
		name: 'my-select',
		multiple: false,
		options: myOptions,
	});

	return (
		<div>
			<SelectSearch
				formId='my-form'
				name='my-select'
				label='My Select'
				options={myOptions}
			/>

			<button onClick={() => selectByValue('js')}>Выбрать JavaScript</button>
			<button onClick={() => clearAll()}>Очистить все</button>
		</div>
	);
}
```

### Другие доступные хуки

```tsx
// Управление состоянием поля
import { useSelectStoreField } from 'wma-form';

// Загрузка опций с кэшированием
import { useOptionsLoader } from 'wma-form';

// Управление дропдауном
import { useDropdownManager } from 'wma-form';

// Обработчики событий
import { useSelectHandlers } from 'wma-form';

// Позиционирование дропдауна
import { useDropdownPosition } from 'wma-form';

// Доступность (a11y)
import { useSelectAccessibility } from 'wma-form';
```

## 🔄 Асинхронная загрузка данных

### Базовый пример

```tsx
const loadOptions = async (query: string) => {
	// Компонент автоматически показывает "Loading..." во время запроса
	const response = await fetch(`/api/search?q=${encodeURIComponent(query)}`);
	const data = await response.json();

	return data.map((item) => ({
		label: item.name,
		value: item.id,
	}));
};

<SelectSearch
	formId='form'
	name='users'
	label='Поиск пользователей'
	loadOptions={loadOptions}
	debounceMs={300} // Задержка перед запросом
	visibleCount={10} // Максимум опций в дропдауне
/>;
```

### Продвинутые возможности

```tsx
// Поиск с отменой предыдущих запросов
let currentController: AbortController | null = null;

const loadOptions = async (query: string) => {
	// Отменяем предыдущий запрос
	if (currentController) {
		currentController.abort();
	}

	currentController = new AbortController();

	try {
		const response = await fetch(`/api/search?q=${query}`, {
			signal: currentController.signal,
		});

		const data = await response.json();
		return data;
	} catch (error) {
		if (error.name === 'AbortError') {
			return []; // Запрос отменен
		}
		throw error;
	}
};

// Поиск с кэшированием
const cache = new Map();

const loadOptions = async (query: string) => {
	if (cache.has(query)) {
		return cache.get(query);
	}

	const response = await fetch(`/api/search?q=${query}`);
	const data = await response.json();

	cache.set(query, data);
	return data;
};
```

## 🎨 Стилизация

### CSS переменные

Компонент использует CSS переменные для полной кастомизации:

```css
:root {
	/* Цвета дропдауна */
	--background-color-select-search-dropdown: #ffffff;
	--border-color-select-search-dropdown: #e0e0e0;
	--color-select-search-dropdown-option: #333333;
	--color-select-search-dropdown-option-selected: #1976d2;
	--color-select-search-dropdown-option-active: #f5f5f5;

	/* Цвета тегов */
	--background-color-select-search-dropdown-tag: #1976d2;
	--color-select-search-dropdown-tag: #ffffff;
	--background-color-select-search-dropdown-tag-placeholder: #f5f5f5;

	/* Размеры и отступы */
	--font-size-option: 14px;
	--font-size-tag: 14px;
	--radius-select-search-dropdown: 8px;
	--padding-select-search-dropdown-item-option-TB: 12px;
	--padding-select-search-dropdown-item-option-LR: 16px;

	/* Анимации */
	--transition-select-search-dropdown: 0.2s ease;
}
```

### Кастомные стили

```css
/* Кастомный дропдаун */
.my-custom-select .dropdown {
	border-radius: 12px;
	box-shadow: 0 8px 24px rgba(0, 0, 0, 0.15);
	border: 2px solid #e3f2fd;
}

/* Градиентные теги */
.my-custom-select .selected-tag {
	background: linear-gradient(45deg, #ff6b6b, #4ecdc4);
	color: white;
	border: none;
}

/* Анимированные опции */
.my-custom-select .dropdown-option {
	transition: all 0.2s ease;
}

.my-custom-select .dropdown-option:hover {
	background-color: #f0f8ff;
	transform: translateX(4px);
	box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);
}

/* Кастомный индикатор загрузки */
.my-custom-select .dropdown-loading::before {
	border-color: #ff6b6b;
	border-top-color: #4ecdc4;
}
```

## ⌨️ Клавиатурная навигация

Компонент полностью поддерживает клавиатурную навигацию:

-   **Arrow Down/Up** - навигация по опциям
-   **Enter** - выбор активной опции
-   **Escape** - закрытие дропдауна
-   **Tab** - переход к следующему элементу
-   **Backspace** - удаление последнего выбранного элемента (в множественном режиме)

## 🏆 Лучшие практики

### 1. Производительность

```tsx
// ✅ Мемоизация опций
const memoizedOptions = useMemo(
	() =>
		users.map((user) => ({
			label: user.name,
			value: user.id,
		})),
	[users]
);

// ✅ Оптимальная задержка для debounce
<SelectSearch
	loadOptions={searchUsers}
	debounceMs={300} // Не слишком быстро, не слишком медленно
/>;
```

### 2. Доступность

```tsx
// ✅ Семантические лейблы
<SelectSearch
	label='Выберите страну проживания'
	name='country'
	required={true}
/>;

// ✅ Понятные сообщения об ошибках
const validator = ({ value }) => {
	if (!value) return 'Пожалуйста, выберите страну';
	return '';
};
```

### 3. UX

```tsx
// ✅ Понятные плейсхолдеры
<SelectSearch
  placeholder="Начните вводить название города..."
  multiple={false}
/>

// ✅ Ограничение видимых опций для производительности
<SelectSearch
  visibleCount={10}
  loadOptions={searchCities}
/>

// ✅ Скрытие выбранных в множественном выборе
<SelectSearch
  multiple={true}
  hideSelectedFromDropdown={true}
  maxVisibleItems={3}
/>
```

### 4. Обработка ошибок

```tsx
const loadOptions = async (query: string) => {
	try {
		const response = await fetch(`/api/search?q=${query}`);
		if (!response.ok) {
			throw new Error(`HTTP ${response.status}`);
		}
		return await response.json();
	} catch (error) {
		console.error('Ошибка поиска:', error);
		return []; // Возвращаем пустой массив при ошибке
	}
};
```

## 🐛 Troubleshooting

### Частые проблемы

#### Дропдаун не открывается

```tsx
// ❌ Проблема: Не указан formId
<SelectSearch name="select" label="Select" />

// ✅ Решение: Указать formId
<SelectSearch formId="my-form" name="select" label="Select" />
```

#### Валидация не работает

```tsx
// ❌ Проблема: required={false}
<SelectSearch
  formId="form"
  name="select"
  label="Select"
  validator={validator}
  required={false} // Валидация не сработает
/>

// ✅ Решение: required={true}
<SelectSearch
  formId="form"
  name="select"
  label="Select"
  validator={validator}
  required={true}
/>
```

#### Асинхронная загрузка не работает

```tsx
// ❌ Проблема: Неправильная сигнатура
const loadOptions = (query) => {
	return fetch(`/api/search?q=${query}`); // Возвращает Promise<Response>
};

// ✅ Решение: Правильная сигнатура
const loadOptions = async (query: string) => {
	const response = await fetch(`/api/search?q=${query}`);
	const data = await response.json();
	return data.map((item) => ({
		label: item.name,
		value: item.id,
	}));
};
```

## 📝 Changelog

### v1.0.0

-   Первоначальный релиз
-   Поддержка одиночного и множественного выбора
-   Асинхронная загрузка данных с кэшированием
-   Внешнее управление через хуки
-   Полная поддержка доступности (a11y)
-   Кастомизируемые стили через CSS переменные
-   Клавиатурная навигация
-   Умные теги с overflow

## 🤝 Поддержка

Если у вас есть вопросы или проблемы:

1. Проверьте раздел [Troubleshooting](#troubleshooting)
2. Изучите примеры в Stories
3. Создайте issue в репозитории

## 📄 Лицензия

MIT License
