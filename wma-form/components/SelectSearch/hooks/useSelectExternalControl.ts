...
interface UseSelectExternalControlOptions {
	formId: string;
	name: string;
	multiple?: boolean;
	options?: Option[];
...
/**
 * Улучшенный хук для внешнего управления селектом
 * Взято ВСЕ из Simple версии + добавлены недостающие методы
 */
export function useSelectExternalControl({ formId, name, multiple = false, options = [], loadOptions, initialValue, debounceMs = 250 }: UseSelectExternalControlOptions) {
	// Refs
	...
	// Внешние функции управления (ВСЕ из Simple версии)
	...
	// ДОПОЛНИТЕЛЬНЫЕ МЕТОДЫ (которых не было в Simple)

	/**
	 * Выбрать несколько опций сразу
	 */
	const selectMultiple = React.useCallback(
		(values: (string | number)[]) => {
			if (multiple) {
				values.forEach((value) => selectByValue(value));
			}
		},
		[multiple, selectByValue]
	);

	/**
	 * Удалить конкретную опцию
	 */
	const removeOption = React.useCallback(
		(value: string | number) => {
			if (multiple) {
				selectByValue(value); // В Simple версии это переключает опцию
			} else {
				clearAll();
			}
		},
		[multiple, selectByValue, clearAll]
	);

	/**
	 * Получить текущий поисковый запрос
	 */
	const getCurrentQuery = React.useCallback(() => {
		return query;
	}, [query]);

	/**
	 * Проверить, открыт ли дропдаун
	 */
	const isDropdownOpen = React.useCallback(() => {
		return dropdownManager.open;
	}, [dropdownManager.open]);

	/**
	 * Установить фокус на инпут (алиас для focusField)
	 */
	const focusInput = focusField;

	/**
	 * Убрать фокус с инпута (алиас для blurField)
	 */
	const blurInput = blurField;

	/**
	 * Отметить поле как focused
	 */
	const markAsFocused = React.useCallback(
		(focused: boolean) => {
			setFieldFocused(formId, name, focused);
		},
		[setFieldFocused, formId, name]
	);
	...
	/**
	 * Получить все опции (для совместимости)
	 */
	...
	/**
	 * Получить отфильтрованные опции (для совместимости)
	 */
	...
	return {
		// Основные функции управления (из Simple)
		selectByValue,
		clearAll,
		openDropdown,
		closeDropdown,
		...
		// Управление фокусом (из Simple)
		focusField,
		blurField,

		// Валидация и состояние (из Simple)
		...
		// Внутренние состояния (из Simple)
		field: safeField,
		selectedOptions,
		selectedLabel,
		selectedOption,
